...
export async function POST(request: NextRequest) {
  ...
  if (role === 'presenter') {
    if (presenter.id === null) {
      ...
      // Não altera currentSlide aqui
      return NextResponse.json({ presenterId: newPresenterId, currentSlide: presenter.currentSlide }, { status: 200 });
    } else if (presenterId && presenter.id === presenterId) {
      if (currentSlide !== undefined) {
        // Atualiza o slide apenas se explicitamente enviado
